...
class CachingSupport<T> implements Serializable {
	...
	public boolean isCachingPossible(EntityContainer<T> container) {
		if (container != null && container.getQueryModifierDelegate() != null) {
			QueryModifierDelegate delegate = container.getQueryModifierDelegate();
			if (delegate instanceof QueryModifierDelegate.CacheAware)
				return ((QueryModifierDelegate.CacheAware) delegate).isCachingPermitted();
		}
		QueryModifierDelegate d = entityProvider.getQueryModifierDelegate();
		if (d != null) {
			if (d instanceof QueryModifierDelegate.CacheAware)
				return ((QueryModifierDelegate.CacheAware) d).isCachingPermitted();
